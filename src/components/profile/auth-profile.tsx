...
export function AuthProfile() {
  ...
  const signIn = async () => { setBusy(true); const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/profile/` } }); setMessage(error ? error.message : "Đã gửi Magic Link. Hãy kiểm tra email để đăng nhập."); setBusy(false); };
